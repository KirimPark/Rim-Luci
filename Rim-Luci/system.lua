...
m = Map("system", translate("시스템"), translate("장치의 Hostname 이나 시간대 등 장치의 기본 요소를 구성할 수 있습니다."))
m:chain("luci")


s = m:section(TypedSection, "system", translate("시스템 속성"))
s.anonymous = true
s.addremove = false

s:tab("general",  translate("일반 설정"))
...
o = s:taboption("general", DummyValue, "_systime", translate("현지 시간"))
...
o = s:taboption("general", ListValue, "zonename", translate("시간대"))
o:value("Asia/Seoul")
...
if has_ntpd then
	...
	if not has_section then
		...
		s = m:section(TypedSection, "timeserver", translate("시간 동기화"))
		s.anonymous   = true
