...
function index()
	entry({"admin", "system"}, alias("admin", "system", "system"), _("시스템"), 30).index = true
	entry({"admin", "system", "system"}, cbi("admin_system/system"), _("시스템"), 1)
	entry({"admin", "system", "clock_status"}, call("action_clock_status"))

	entry({"admin", "system", "admin"}, cbi("admin_system/admin"), _("관리"), 2)

	if nixio.fs.access("/bin/opkg") then
		entry({"admin", "system", "packages"}, call("action_packages"), _("소프트 웨어"), 10)
		entry({"admin", "system", "packages", "ipkg"}, form("admin_system/ipkg"))
	end

	entry({"admin", "system", "startup"}, form("admin_system/startup"), _("시작"), 45)
	entry({"admin", "system", "crontab"}, form("admin_system/crontab"), _("예약된 작업"), 46)
	...
	if nixio.fs.access("/sys/class/leds") then
		entry({"admin", "system", "leds"}, cbi("admin_system/leds"), _("<abbr title=\"Light Emitting Diode\">LED</abbr> 구성"), 60)
	end

	entry({"admin", "system", "flashops"}, call("action_flashops"), _("Backup / Flash Firmware"), 70)
	entry({"admin", "system", "flashops", "backupfiles"}, form("admin_system/backupfiles"))

	entry({"admin", "system", "reboot"}, call("action_reboot"), _("재부팅"), 90)
	
	entry({"admin", "system", "sendnfc"}, cbi("sendnfc/sendnfc"), _("SendNfc"), 91)
